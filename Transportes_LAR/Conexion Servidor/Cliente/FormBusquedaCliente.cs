...
namespace Transportes_LAR.Interfaz.Cliente
{
	public partial class FormBusquedaCliente : Form
	{
		...
		void FormBusquedaClienteLoad(object sender, EventArgs e){
			dataCliente.DataSource=new Conexion_Servidor.Cliente.SQL_CLiente().getTabla("select ID,Empresa,SubNombre FROM Cliente where Empresa!='Especiales' order by Empresa");
			getDatoCliente();
		}
		...
		void BtnAgreagarClick(object sender, EventArgs e){
			...
			dataCliente.DataSource=new Conexion_Servidor.Cliente.SQL_CLiente().getTabla("select ID,Empresa,SubNombre FROM Cliente");
			getDatoCliente();
		}
