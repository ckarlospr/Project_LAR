...
namespace Transportes_LAR.Interfaz.Combustible.EasyTrack
{
	public partial class FormDatos : Form
	{
		...
		void getTablas()
		{
			...
			String consulta = "SELECT * FROM transportes_LAR.INFORMATION_SCHEMA.TABLES where TABLE_NAME like '%mac%' and TABLE_NAME != 'MAC_OPERACION' and TABLE_NAME != 'MAC_UNIDAD'";
		...
		void getdgMacUnidad()
		{
			...
			String consulta = "select ID, Numero from VEHICULO where estatus='1' and (Tipo_Unidad='Camión' or Tipo_Unidad='Camioneta') order by Numero";
			conn.getAbrirConexion(consulta);
			conn.leer=conn.comando.ExecuteReader();
			while(conn.leer.Read())
			{
