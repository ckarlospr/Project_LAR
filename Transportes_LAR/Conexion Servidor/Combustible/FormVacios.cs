...
namespace Transportes_LAR.Interfaz.Combustible
{
	public partial class FormVacios : Form
	{
		...
		{
			for(int x=0; x<refPrincipal.dgValidado.Rows.Count; x++)
			{
			...
			refPrincipal.Valida(Convert.ToInt64(dgVacios[0,dgVacios.CurrentRow.Index].Value), Convert.ToInt64(refPrincipal.dgAutorizacionValida[11,refPrincipal.dgAutorizacionValida.CurrentRow.Index].Value),"VACIO", refPrincipal.txtComentario.Text, dtpFecha.Value.ToString("dd/MM/yyyy"), "00:00");
