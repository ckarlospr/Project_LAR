...
namespace Transportes_LAR.Conexion_Servidor.Desapacho
{
	public class SQL_Operaciones:Conexion_Servidor.SQL_Conexion
	{
		...
		{
			base.getAbrirConexion("update GUARDIA set  ID_OPERADOR="+idOP+", tipo='0'  where ID="+idOH);
			base.comando.ExecuteNonQuery();
			base.conexion.Close();
		}
		#endregion
		...
		#region OBTENER_RUTAS_SALIDA
		public List<Interfaz.Operaciones.Dato.Apoyo> getListApoyoRuta_Op(String turno, String dia, String empresa)
		{
			List<Interfaz.Operaciones.Dato.Apoyo> listApoyo = new List<Transportes_LAR.Interfaz.Operaciones.Dato.Apoyo>();
			
//			String Consulta = ";WITH cte_data as (select OE.NOMBRE empresa, R.Nombre, R.Sentido, P.Alias, count(1) as number "
//								+	",row_number() over(partition by OE.NOMBRE, R.Nombre, R.Sentido order by count(1) desc) as 'numero' "
//								+"from ORDEN_EMPRESAS OE, CLIENTE C, RUTA R, OPERACION O, OPERACION_OPERADOR OP , OPERADOR P  "
//								+"where C.ID_ORDEN=OE.ID AND C.ID=R.IdSubEmpresa and R.ID=O.id_ruta and P.Estatus=1  "
//								+"	and O.id_operacion=OP.id_operacion and OP.id_operador=P.ID and O.turno='"+turno+"' "
//								+"	and OE.NOMBRE!='ESPECIALES' and OE.nombre='"+empresa+"' and O.fecha>DATEADD(MONTH, -2, GETDATE()) "
//								+"group by OE.NOMBRE, R.Nombre, R.Sentido, P.Alias) "
//								+"select * from cte_data where numero=1";
			
			String Consulta = ";WITH cte_data as (select OE.NOMBRE empresa, R.Nombre, R.Sentido, P.Alias, count(1) as number "
								+"	,row_number() over(partition by OE.NOMBRE, R.Nombre, R.Sentido order by count(1) desc) as 'numero' "
								+"from ORDEN_EMPRESAS OE "
								+"	INNER JOIN CLIENTE C ON C.ID_ORDEN=OE.ID "
								+"	INNER JOIN RUTA R ON C.ID=R.IdSubEmpresa "
								+"	INNER JOIN OPERACION O ON R.ID=O.id_ruta "
								+"	INNER JOIN OPERACION_OPERADOR OP ON O.id_operacion=OP.id_operacion "
								+"	INNER JOIN OPERADOR P ON OP.id_operador=P.ID "
								+"WHERE O.turno='"+turno+"' AND P.Estatus=1 and O.estatus=1 and OE.NOMBRE!='ESPECIALES' and OE.nombre='"+empresa+"' and O.fecha>DATEADD(DAY, -6, GETDATE()) "
								+"group by OE.NOMBRE, R.Nombre, R.Sentido, P.Alias) "
								+"select * from cte_data where numero=1";
			...
			while(base.leer.Read())
			{
				...
				apoyo.Empresa	=base.leer["empresa"].ToString();
		...
		#region APOYOS
		public void guardarApoyo(int id_u, Int64 id_operador, int id_apoya, Int64 id_ruta, String dia, String turno, String tipo, String comentarios)
		{
			try
			{
		...
		public List<Interfaz.Operaciones.Dato.Operador> getapoyos(String dia, String turno)
		{
			...
			//base.getAbrirConexion("select a.ID ,o.Alias,a.TIPO,  a.COMENTARIOS, a.DIA, r.Nombre  as RUTA, c.Empresa from APOYOS a, RUTA r, OPERADOR o, Cliente c where a.ID_RUTA = r.ID and r.IdSubEmpresa = c.ID and a.ID_OP_APOYO = o.ID and  a.DIA = '"+dia+"' and a.TURNO = '"+turno+"'");
			base.getAbrirConexion("select a.ID , o.Alias,a.TIPO,  a.COMENTARIOS, a.DIA, u.usuario, c.Empresa, r.Nombre as RUTA, c.Empresa from APOYOS a, RUTA r, OPERADOR o, Cliente c, usuario u where a.ID_U = u.id_usuario and a.ID_RUTA = r.ID and r.IdSubEmpresa = c.ID and a.ID_OP_APOYO = o.ID and  a.DIA = '"+dia+"' and a.TURNO = '"+turno+"'");
			...
			while(base.leer.Read())
			{
				...
				dat.ruta        =base.leer["Ruta"].ToString();
				dat.usuario     =base.leer["usuario"].ToString();
				dat.empresa     =base.leer["Empresa"].ToString();
