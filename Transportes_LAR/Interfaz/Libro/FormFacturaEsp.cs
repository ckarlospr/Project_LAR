...
namespace Transportes_LAR.Interfaz.Libro
{
	public partial class FormFacturaEsp : Form
	{
		...
		{
			consulta="select RE.ID_RE, RE.FACTURA, RE.DESTINO, CS.Nombre, RE.RESPONSABLE, RE.CANT_UNIDADES, RE.tipoVehiculo, RE.FACTURADO, RE.FECHA_SALIDA from RUTA_ESPECIAL RE, ContactoServicio CS where RE.ID_C=CS.IdCliente and RE.ID_C in (select IdSubEmpresa from RUTA) ";
		}
		...
		public void getDatos()
		{
			...
			{
				dgEspeciales.Rows.Add(conn.leer["ID_RE"].ToString(),conn.leer["FACTURA"].ToString(),conn.leer["DESTINO"].ToString(),conn.leer["Nombre"].ToString(), conn.leer["RESPONSABLE"].ToString(), conn.leer["FECHA_SALIDA"].ToString().Substring(0,10), conn.leer["CANT_UNIDADES"].ToString(), conn.leer["tipoVehiculo"].ToString(), conn.leer["FACTURADO"].ToString());
			}
		...
		public void guardaDatos(String DATO)
		{
			...
			{
				conn.getAbrirConexion("UPDATE RUTA_ESPECIAL SET FACTURADO='3', NUMERO_ANTI='"+DATO+"'  WHERE ID_RE="+dgEspeciales[0,dgEspeciales.CurrentRow.Index].Value.ToString());
