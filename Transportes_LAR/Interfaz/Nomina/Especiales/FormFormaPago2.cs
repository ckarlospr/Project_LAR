...
{
	public partial class FormFormaPago2 : Form
	{
		public FormFormaPago2(FormMalFlujo refPrinc)
		{
			InitializeComponent();
		...
		#region INSTANCIAS
		private FormMalFlujo refPrincipal;
						...
						if(rbEfectivo.Checked==true)
						{
							...
							{*/
								...
								{									
									...
									conn.conexion.Close();
									MessageBox.Show("Servicio Pagado", "Listo",MessageBoxButtons.OK, MessageBoxIcon.Information);
								}
							//}
		...
		#region INICIO - FIN		
		void FormFormaPago2FormClosing(object sender, FormClosingEventArgs e)
		{
			
		}
		
		void FormFormaPago2Load(object sender, EventArgs e)
		{
			for(int x=0; x<refPrincipal.dgRelacion.Rows.Count; x++)
				{
					if(refPrincipal.dgRelacion[2,x].Selected==true)
					{
						lblID.Text = "ID del Servicio: "+refPrincipal.dgRelacion[0,x].Value.ToString();
					}
			}
		}
		#endregion
