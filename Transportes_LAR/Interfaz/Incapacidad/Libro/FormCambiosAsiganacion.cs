...
namespace Transportes_LAR.Interfaz.Libro
{
	public partial class FormCambiosAsiganacion : Form
	{
		...
		{
			new Conexion_Servidor.Desapacho.SQL_Operaciones().deleteAsignacion2(id_operacion);
		}
		#endregion
		...
		void TxtEntradaKeyUp(object sender, KeyEventArgs e)
		{
			...
			{
				String consul = "select ID from OPERADOR where Alias='"+txtEntrada.Text+"'";
				conn.getAbrirConexion(consul);
				conn.leer=conn.comando.ExecuteReader();
				if(conn.leer.Read())
				{
		...
		void TxtSalidaKeyUp(object sender, KeyEventArgs e)
		{
			...
			if(e.KeyCode == Keys.Return)
			{
				String consul = "select * from OPERADOR where Alias='"+txtSalida.Text+"'";
				conn.getAbrirConexion(consul);
				conn.leer=conn.comando.ExecuteReader();
				if(conn.leer.Read())
				{
