...
namespace Transportes_LAR.Interfaz.Libro
{
	public partial class FormAsignacionCobro : Form
	{
		...
		void TxtCobroKeyUp(object sender, KeyEventArgs e)
		{
			...
			{
				String consul = "select T.Numero from TELEFONOCHOFER T, OPERADOR O where T.Descripcion='Particular' and T.ID_Chofer=O.ID AND O.Alias='"+txtCobro.Text+"'";
		...
		void CmdAceptarClick(object sender, EventArgs e)
		{
			if(txtTelefono.Text!="")
			{
				...
				else if(TIPO==2)
				{
					...
					formValidacion.guardaValidacion(formValidacion.dgDatos[3,formValidacion.dgDatos.CurrentRow.Index].Value.ToString(),formValidacion.dgDatos[1,formValidacion.dgDatos.CurrentRow.Index].Value.ToString(),formValidacion.dgDatos[2,formValidacion.dgDatos.CurrentRow.Index].Value.ToString(), tipoCobro, txtCobro.Text, txtComentario.Text, txtTelefono.Text, formValidacion.dgDatos[0,formValidacion.dgDatos.CurrentRow.Index].Value.ToString(), formValidacion.dgDatos[21,formValidacion.dgDatos.CurrentRow.Index].Value.ToString(), txtSaldo.Text);
