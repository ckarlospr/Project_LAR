...
{
	public partial class FormRecuperado : Form
	{
		#region CONSTRUCTOR
		public FormRecuperado(String id_re, FormValidarServicios rep)
		{
			InitializeComponent();
			refReporte = rep;
			ID = id_re;
		...
		#region INICIO - FIN
		void FormRecuperadoLoad(object sender, EventArgs e)
		{
			obtenerDatos();
			this.txtCambio.KeyPress += new KeyPressEventHandler(Proceso.ValidarCampo.soloNumeros);			
		...
		private void obtenerDatos(){
			consulta = @"select C.ID_RE, C.COBRO, O.Alias, C.TELEFONO, RE.PRECIO, RE.anticipo, RE.DESTINO, RE.FACTURA, RE.FECHA_SALIDA, C.IDENTIFICADOR, C.FECHA, 
						C.PAGO, C.FACTURA fact, C.BORRADO, C.INCOBRABLE, C.TIPO_VIAJE, V.Numero, RE.NUMERO_ANTI, C.SALDO, C.ID IDCC, C.OBSERVACIONES, RE.OP_COBRA, 
						C.TIPO from COBRO_ESPECIAL C, RUTA_ESPECIAL RE, VEHICULO V, OPERADOR O where O.ID=C.ID_OP AND C.ID_VEHICULO=V.ID AND C.ID_RE=RE.ID_RE and C.ESTATUS='1' AND C.ID_RE = "+ID;
			conn.getAbrirConexion(consulta);
			conn.leer=conn.comando.ExecuteReader();
			while(conn.leer.Read()){
				dgCobros.Rows.Add(conn.leer["IDCC"].ToString(), 
				                  ...
				                  conn.leer["TIPO_VIAJE"].ToString(), 
				                  conn.leer["OP_COBRA"].ToString(), 
		...
		void CmdAplicaClick(object sender, EventArgs e)
		{
			if(dgCobros.SelectedRows.Count > 0){
				if(dgCobros[5,dgCobros.CurrentRow.Index].Value.ToString() == "SI"){
					MessageBox.Show("Servicio ya pagado.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
				}else{
					//String sentencia = "update COBRO_ESPECIAL set PAGO = '1', SALDO = '0' where ID='"+dgCobros[0,dgCobros.CurrentRow.Index].Value.ToString()+"'";
					String sentencia = "update COBRO_ESPECIAL set PAGO = '1',  TIPO = 'PAGADO'  where ID='"+dgCobros[0,dgCobros.CurrentRow.Index].Value.ToString()+"'";
					conn.getAbrirConexion(sentencia);
					conn.comando.ExecuteNonQuery();
					conn.conexion.Close();
					dgCobros[5,dgCobros.CurrentRow.Index].Value="SI";
		...
		void CmdCambioClick(object sender, EventArgs e)
		{
			if(dgCobros.SelectedRows.Count > 0){
				if(dgCobros[5,dgCobros.CurrentRow.Index].Value.ToString() == "SI"){
					MessageBox.Show("No se puede modificar.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
				}else{
					...
					refReporte.dgReporte[9,refReporte.dgReporte.CurrentRow.Index].Value = (Convert.ToDouble(refReporte.dgReporte[9,refReporte.dgReporte.CurrentRow.Index].Value)-Convert.ToDouble(dgCobros[4,dgCobros.CurrentRow.Index].Value)).ToString();
					MessageBox.Show("Se cambio el saldo del vehiculo correctamente ", "Listo", MessageBoxButtons.OK, MessageBoxIcon.Information);
				}
			}
		}
		#endregion
