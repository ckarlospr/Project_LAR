...
{
	public partial class FormCabiosValidacion : Form
	{
		#region CONSTUCTOR
		public FormCabiosValidacion(FormLibroViajes refValidacion, int tipo)
		{
			InitializeComponent();	
		...
		#region INSTANCIAS
		private FormLibroViajes refValida = null;
		private Proceso.AutoCompleClass auto= new Transportes_LAR.Proceso.AutoCompleClass();
		private Conexion_Servidor.SQL_Conexion conn= new Conexion_Servidor.SQL_Conexion();
		#endregion
		
		#region INICIO - FIN
		void FormCabiosValidacionLoad(object sender, EventArgs e)
		{
			 getDatos();
		...
		void CmdGuardarClick(object sender, EventArgs e)
		{
			...
			else if(ACCION=="PUNTO")
			{
				if(refValida.dgDatos[10,refValida.dgDatos.CurrentRow.Index].Value.ToString()!="0")
				{
					...
					cancelacionPunto(idOp, Convert.ToInt64(refValida.dgDatos[1,refValida.dgDatos.CurrentRow.Index].Value));
					desasignar(Convert.ToInt64(refValida.dgDatos[10,refValida.dgDatos.CurrentRow.Index].Value));
					desasignar(Convert.ToInt64(refValida.dgDatos[12,refValida.dgDatos.CurrentRow.Index].Value));
					this.Close();
				}
				...
				else if(idOperadoEnt!=0)
				{
					...
					cancelacionPunto(Convert.ToInt16(id_ope), Convert.ToInt32(refValida.dgDatos[1,refValida.dgDatos.CurrentRow.Index].Value));
					desasignar(Convert.ToInt64(refValida.dgDatos[12,refValida.dgDatos.CurrentRow.Index].Value));
					this.Close();
				}
				else
			...
			else if(ACCION=="CANCELA")
			{
				...
				canceladoAnt(Convert.ToInt32(refValida.dgDatos[1,refValida.dgDatos.CurrentRow.Index].Value));
				canceladoAnt(Convert.ToInt32(refValida.dgDatos[2,refValida.dgDatos.CurrentRow.Index].Value));
				cambioCancelAnt();
				this.Close();
		...
		#region METODOS DB
			...
			{
				new Conexion_Servidor.Desapacho.SQL_Operaciones().deleteAsignacion2(id_operacion, refValida.principal.idUsuario);
			}
			#endregion
			...
			void asignacionViaje(Int64 id_ruta, Int64 id_operador, Int64 id_vehiculo, String tipo_vehiculo)
			{
				...
				new Conexion_Servidor.Desapacho.SQL_Operaciones().almacenarViaje(0, id_ruta.ToString(), refValida.principal.idUsuario, id_operador, "00:00", "00:00", 1, id_vehiculo, tipo_vehiculo, "Mañana", 0, " ", fecha.Substring(0,10), "Espsciales");
			}
			#endregion
			
			#region CANCELADO EN EL PUNTO
			void cancelacionPunto(int id_operador, Int64 id_ruta)
			{
				string fecha="";
				...
				new Conexion_Servidor.Desapacho.SQL_Operaciones().cancelaPunto(id_operador, id_ruta, refValida.principal.idUsuario, "Mañana", fecha.Substring(0,10));
			}
			#endregion
			
			#region CANCELADO ANTICIPADO
