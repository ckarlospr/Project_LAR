...
{
	public partial class FormValidacionServicios : Form
	{
		#region CONSTRUCTOR
		public FormValidacionServicios(FormLibroViajes refLib)
		{
			InitializeComponent();
			refLibro=refLib;
		}
		...
		#region INICIO - FIN
		void FormValidacionServiciosLoad(object sender, EventArgs e)
		{
			...
			obternerConsulta();
			consulta=consulta+" and FECHA_SALIDA between '2013/02/12' and '"+dtpFin.Value.ToString("dd/MM/yyyy")+"' and R.PAGO='0' ";//DateTime.Now.ToString("yyyy/MM/dd")+"' and R.PAGO='0' "; //
			getDatosDatagird();
			getSalidas();
		}		
		#endregion
		...
		int filaSelect=0;
		public List<Libro_Nuevo.Dato.datosViajes> listRutaSalida = null;
		#endregion
		...
		#region LLENADO DE DATAGRID (dgDatos)
			public void getDatosDatagird()
			{
				...
				{
					dgDatos.Rows.Add(conn.leer["estado"].ToString(),conn.leer["ID"].ToString(), "0",conn.leer["ID_RE"].ToString(),conn.leer["DESTINO"].ToString(),"$"+conn.leer["PRECIO"].ToString(), "$"+conn.leer["ANTICIPO"].ToString(), ((conn.leer["FACTURADO"].ToString()!="0")?(Convert.ToDouble(conn.leer["PRECIO"])*1.16).ToString():conn.leer["PRECIO"].ToString()),conn.leer["FECHA_SALIDA"].ToString().Substring(0,10), "OPERADOR", "0", "OPERADOR", "0", "OPERADOR", "000000000", "N/A", "N/A", "N/A", "N/A", "OPERADOR", conn.leer["PAGO"].ToString(), "", "", "","167");
			...
			{
				consulta="select R.ID, RE.ID_RE, RE.DESTINO, RE.PRECIO, RE.anticipo, RE.SALDO, RE.FECHA_SALIDA, RE.estado, R.PAGO, RE.FACTURADO from RUTA R, Cliente C, RUTA_ESPECIAL RE where R.IdSubEmpresa=C.ID and C.ID=RE.ID_C AND R.Sentido='ENTRADA' ";
			...
			public void datosCompleta()
			{
				for(int x=0; x<dgDatos.Rows.Count; x++)
				{
					if(dgDatos[23,x].Value.ToString()!="0")
					{
						// para iva ********************
					}
					
					String consul = "select OP.Alias, O.id_operacion, R.PAGO FLUJO, OO.id_vehiculo, OO.vehiculo, OP.ID from RUTA R, Cliente C, RUTA_ESPECIAL RE, OPERACION O, OPERACION_OPERADOR OO, OPERADOR OP where R.IdSubEmpresa=C.ID and C.ID=RE.ID_C and O.id_ruta=R.ID and O.id_operacion=OO.id_operacion and OO.id_operador=OP.ID and O.estatus='1' and R.ID="+dgDatos[1,x].Value.ToString();
					...
					if(conn.leer.Read())
					{
						...
						dgDatos[24,x].Value=conn.leer["ID"].ToString();
					...
					consul = "select OP.Alias, O.id_operacion, R.PAGO FLUJO, OO.id_vehiculo, OO.vehiculo, OP.ID from RUTA R, Cliente C, RUTA_ESPECIAL RE, OPERACION O, OPERACION_OPERADOR OO, OPERADOR OP where R.IdSubEmpresa=C.ID and C.ID=RE.ID_C and O.id_ruta=R.ID and O.id_operacion=OO.id_operacion and OO.id_operador=OP.ID and O.estatus='1' and R.ID="+dgDatos[2,x].Value.ToString();
					conn.getAbrirConexion(consul);
					conn.leer=conn.comando.ExecuteReader();
					if(conn.leer.Read())
					{
					...
					consul = "select O.Alias, V.ID, V.Tipo_Unidad, O.ID ID_OP from CANCELACION_PUNTO C, OPERADOR O, ASIGNACIONUNIDAD A, VEHICULO V WHERE O.ID=A.ID_OPERADOR and A.ID_UNIDAD=V.ID and C.ID_OPERADOR=O.ID and ID_RUTA="+dgDatos[1,x].Value.ToString();
					conn.getAbrirConexion(consul);
					conn.leer=conn.comando.ExecuteReader();
					if(conn.leer.Read())
					{
						...
						if(dgDatos[10,x].Value.ToString()=="0")
						{
							dgDatos[9,x].Value=conn.leer["Alias"].ToString();
							dgDatos[13,x].Value=conn.leer["Alias"].ToString();
							dgDatos[21,x].Value=conn.leer["ID"].ToString();
							dgDatos[22,x].Value=conn.leer["Tipo_Unidad"].ToString();
							dgDatos[24,x].Value=conn.leer["ID_OP"].ToString();
						}
						else
						{
					...
					consul = "select T.Numero from TELEFONOCHOFER T, OPERADOR O where T.Tipo='LAR' and T.ID_Chofer=O.ID AND O.Alias='"+((dgDatos[11,x].Value.ToString().Contains("Error")==true)?dgDatos[11,x].Value.ToString().Substring(8,(dgDatos[11,x].Value.ToString().Length-8)):dgDatos[11,x].Value.ToString())+"'";
					conn.getAbrirConexion(consul);
					conn.leer=conn.comando.ExecuteReader();
					if(conn.leer.Read())
					{
		...
		public void valida()
		{
			if(rbDefault.Checked==true)
			{
				...
				if(dgDatos.SelectedRows.Count>0)
				{
					for(int x=0; x<dgDatos.Rows.Count; x++)
					{
						...
						{
							guardaValidacion(dgDatos[3,x].Value.ToString(),dgDatos[1,x].Value.ToString(), dgDatos[2,x].Value.ToString(), "OPERADOR", dgDatos[13,x].Value.ToString(), " ", dgDatos[14,x].Value.ToString(), dgDatos[0,x].Value.ToString(), dgDatos[21,x].Value.ToString(), dgDatos[7,x].Value.ToString(), dgDatos[24,x].Value.ToString());
							dgDatos[15,x].Value="SI";
							dgDatos[15,x].Style.BackColor=Color.SpringGreen;
							dgDatos[17,x].Value="SI";
							dgDatos[17,x].Style.BackColor=Color.SpringGreen;
			...
			else if(rbDefault.Checked==false)
			{
				...
				{
					new Libro_Nuevo.Pagos.FormInsertarCobro(this,1).ShowDialog();
				}
				else if(dgDatos.SelectedRows.Count==1)
				{
					new Libro_Nuevo.Pagos.FormInsertarCobro(this,2).ShowDialog();
				}
				else
				{
					MessageBox.Show("Seleccione un registro de la tabla.");
			...
			for(int x=0; x<dgDatos.Rows.Count; x++)
			{
				if(dgDatos.Rows[x].Selected==true)
				{
					if(dgDatos.Rows[x].Selected==true && (dgDatos[0,x].Value.ToString()!="CANCELADA" && dgDatos[0,x].Value.ToString()!="SIN CONF."))
					{
						...
						if(guardaValidacion(dgDatos[3,x].Value.ToString(),dgDatos[1,x].Value.ToString(),dgDatos[2,x].Value.ToString(), tipo, cobro, observaciones, telefono, dgDatos[0,x].Value.ToString(), dgDatos[21,x].Value.ToString(), dgDatos[7,x].Value.ToString(), dgDatos[24,x].Value.ToString()))
						{
							dgDatos[15,x].Value="SI";
							dgDatos[15,x].Style.BackColor=Color.SpringGreen;
							dgDatos[17,x].Value="SI";
		...
		public bool guardaValidacion(String ID_RE, String id_r1, String id_r2, String tipo, String cobro, String observaciones, String telefono, String Tipo_V, String id_vehiculo, String saldo, string id_operador)
		{
			...
			try
			{
				miconsult="insert into COBRO_ESPECIAL values ('"+ID_RE+"','"+id_r1+"', '"+id_r2+"', '"+tipo+"', '"+cobro+"', '"+observaciones+"', "+IDENT+", '"+telefono+"', '1', (SELECT CONVERT (date, SYSDATETIME())), "+refLibro.principal.idUsuario+", '"+IDENT+"-"+DateTime.Now.ToString("yyMM")+"', '0', '0', '0', '0', '"+Tipo_V+"', '"+id_vehiculo+"', '"+saldo+"', '', "+id_operador+")";	//
				conn.getAbrirConexion(miconsult);
		...
		void CmdCambiosClick(object sender, EventArgs e)
		{
			if(dgDatos.SelectedRows.Count>0)
			{
				...
				{
					//new FormCabiosValidacion(this, 0).ShowDialog();
				}
				else if(dgDatos[9,dgDatos.CurrentRow.Index].Style.BackColor.Name.ToString()=="Black" || dgDatos[11,dgDatos.CurrentRow.Index].Style.BackColor.Name.ToString()=="Black")
				{
					...
					if (DialogResult.OK==rs2)
					{
						//new FormCabiosValidacion(this, 1).ShowDialog();
					}
				}
				else
				{
